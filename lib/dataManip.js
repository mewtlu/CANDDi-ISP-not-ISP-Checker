...
module.exports.ratioUnwanted = function(stringSet, verbose) {
	...
	var blacklistRegEx = new RegExp('(^[a-zA-Z]?[0-9]{1,}$)|([0-9]{4} [0-9]{5} [0-9]{3})|([0-9]{9,})|([a-z][0-9]{3}[a-z]{2}[0-9]{4,5})|(.*\.rain\.fr)|(.* rue .*)|(.*diwemedia[0-9]{1,2})');
	var whitelistRegEx = new RegExp('^fr ')
	var matched = 0;

	for (var s in stringSet) {
		if (!whitelistRegEx.test(stringSet[s]) && (IPRegEx.test(stringSet[s]) || blacklistRegEx.test(stringSet[s]))) {
